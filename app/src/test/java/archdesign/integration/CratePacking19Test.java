...
public class CratePacking19Test {

    @Test
    void test19PerCrates1_matchesExpectedJson() throws Exception {
        URL inUrl = getClass().getResource("/crate_packing/DifferentSizeSameMedium/pack_long_direction/19_per_crates_1/input.csv");
        assertNotNull(inUrl, "Input resource missing");
        Path inPath = Paths.get(inUrl.toURI());

            ShipmentViewModel vm = null;
            System.setProperty("packing.preferCrates", "true");
            try {
                vm = Main.processFile(inPath.toString());
                ...
                URL expectedUrl = getClass().getResource("/crate_packing/DifferentSizeSameMedium/pack_long_direction/19_per_crates_1/expected_output.json");
