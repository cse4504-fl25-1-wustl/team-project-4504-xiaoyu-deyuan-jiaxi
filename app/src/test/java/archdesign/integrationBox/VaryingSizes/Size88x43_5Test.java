...
/**
 * Test for VaryingSizes/88x43.5
 * ✅ Expected: LARGE box (width 88 > 43, height 43.5 → ceil 44 > 43)
 */
public class Size88x43_5Test {

    private static final String BASE_DIR = "/box_packing/VaryingSizes/88x43.5";

    @Test
    public void test_88x43_5_LargeBox() throws Exception {
        System.out.println(">>> Running test for folder: 88x43.5");

        // 1️⃣ Load resources
        URL inputUrl = getClass().getResource(BASE_DIR + "/input.csv");
        URL expectedUrl = getClass().getResource(BASE_DIR + "/expected_output.json");
        assertNotNull(inputUrl, "input.csv missing in 88x43.5");
        assertNotNull(expectedUrl, "expected_output.json missing in 88x43.5");

        Path inputPath = Paths.get(inputUrl.toURI());
        Path expectedPath = Paths.get(expectedUrl.toURI());

        // 2️⃣ Run main process
        ShipmentViewModel vm = Main.processFile(inputPath.toString());

        // 3️⃣ Load expected output
        ...
        System.out.println("--- Shipment Debug (88x43.5) ---");
